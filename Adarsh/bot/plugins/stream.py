...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("ʟᴏɢɪɴ ғɪʀsᴛ ᴜsɪɴɢ /login ᴄᴍᴅ(ᴛʏᴘᴇ /login)  \n\n password : Ultroidofficial || Donate 10rs to Dm me : @Blank_Blankx \n\n (for running bot need contributions..unwanted message = ban + report)")
            return
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            await broadcast.reply_text("Login first using /login || Password : Ultroidofficial")
            return
        if check_pass != MY_PASS:
            await broadcast.reply_text("Wrong password, login again || for password : @Blank_Blankx msg here ")
